	...
	<table class="table-auto mt-6 w-full text-xs">
		...
		<template v-for="v in list">
			...
			<tr v-else-if="v.state=='已汇总'" style="color: blue">
				...
				<td class="border ">{{v.state}}</td>
				<td class="border ">
					<button v-if="v.mode != '上门收瓶'" @click="cancle(v)" class="bg-teal-500 text-white p-1">取消</button>
				</td>
			</tr>
			<tr v-else>
				<td class="border ">
...
<script>
	...
	new Vue({
		...
		methods: {
			...
			HandleUserDepartmentRepair() {
				...
				axios.post('/index.php/api/HandleUserDepartmentRepair', {
					...
					if (rew.data.code == 200) {
						...
						this.close()
						this.submit()
						var data = rew.data.printinfo.printinfo
						if (data.type == '门店业务') {
			...
			fk(data) {
				if (data.mode == '上门收瓶' || data.mode == '上门收桶') {
					localStorage.setItem('repairdata', JSON.stringify(data));
					Win10_child.openUrl('/index.php/msg/RecyclingMaterials', '门收回用户钢瓶物资');
					return false
				}
				if (data.state != '已安排') {
					swal('该订单无法反馈')
