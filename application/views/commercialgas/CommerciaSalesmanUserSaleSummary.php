...
<div id="app" class="p-6">
	...
	<table class="table-auto mt-6 w-full text-xs">
		...
		<tr>
			...
			<th class=" border-2">门店提付</th>
			<th class=" border-2">门店定付</th>
			<th class=" border-2">运输公司提付</th>
			<th class=" border-2">运输公司定付</th>
			<th class=" border-2">合计</th>
		...
		<tr class="text-center" v-for="v in list">
			...
			<td class="border-2 ">{{ v.mdtf}}</td>
			<td class="border-2 ">{{ v.mddf}}</td>
			<td class="border-2 ">{{ v.ystf}}</td>
			<td class="border-2 ">{{ v.ysdf}}</td>
			<td class="border-2 ">{{ v.hj}}</td>
		</tr>
...
<script>
	...
	new Vue({
		...
		methods: {
			submit() {
				axios.post('/index.php/api/CommerciaSalesmanUserSaleSummary', {
