...
<div id="app" class="p-6">
	...
	<table class="table-auto mt-6 w-full text-xs">
		...
		<tr>
			...
			<th class=" border-2">物品名称</th>
			<th class=" border-2">使用时间</th>
			<th class=" border-2">期初欠</th>
			<th class=" border-2">期间欠</th>
			<th class=" border-2">期间还</th>
			<th class=" border-2">期末欠</th>
		...
		<tr class="text-center" v-for="v in list">
			...
			<td class="border-2 ">{{ v.owename}}</td>
			<td class="border-2 ">{{ v.usetime}}</td>
			<td class="border-2 ">{{ v.qcqp}}</td>
			<td class="border-2 ">{{ v.qjqp}}</td>
			<td class="border-2 ">{{ v.qjhp}}</td>
			<td class="border-2 ">{{ v.qmqp}}</td>
		</tr>
...
<script>
	...
	new Vue({
		...
		methods: {
			submit() {
				axios.post('/index.php/api/CommerciaUserOweCollateralSummary', {
