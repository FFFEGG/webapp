...
<div class="oapd">
	<blockquote class="layui-elem-quote layui-text" style="font-weight: bold;font-size: 20px;color: #222;">
		用户<?= $this->input->get('memberid') ?> 维修记录信息
	</blockquote>
		...
		<table class="layui-table" lay-size="sm">
			...
			<tr>
				<?php foreach ($title as $v) { ?>
					<?php if ($v != '订单号' && $v != '来源' && $v != 'USERID' && $v != '会员号' && $v != '住所类型' && $v != '用户类型' && $v != '街道' && $v != '门牌号') { ?>
						<th><?= $v ?></th>
					<?php } ?>
				<?php } ?>
				<th>操作</th>
				...
				<tr>
					<?php foreach ($key as $vo) { ?>
						<?php if ($vo != 'serial' && $vo != 'source' && $vo != 'street' && $vo != 'housenumber' && $vo != 'userid' && $vo != 'customertype' && $vo != 'memberid' && $vo != 'housingproperty') { ?>
							...
							<?php } elseif ($vo == 'goodsjson') { ?>
								<td>
									<?php foreach (json_decode($v->goodsjson) as $vi) { ?>
										物品:<?= $vi->material ?>  数量:<?= $vi->num ?>  价格:<?= $vi->price ?>  单位:<?= $vi->unit ?>
									<?php } ?>
								</td>
							<?php } else { ?>
								<td><?= $v->$vo ?></td>
					...
					<?php } ?>
					<td>
						<button onclick="seeimg('<?= $v->serial ?>','<?= $v->userid?>')" class="layui-btn layui-btn-sm">查看图片</button>
					</td>
				</tr>
			<?php } ?>
			</tbody>
...
<script>
	...
	function seeimg(serial,userid) {
		Win10_child.openUrl('/index.php/reservationcenter/seeimg?serial='+serial+'&userid='+userid,'查看图片')
	}
</script>
