...
<div id="app" class="p-4">
	...
	<div class="p-3 font-bold grid grid-cols-3 " v-if="user.memberid">
		...
		<div class="p-3 m-2 border-2">地址: {{ user.province }}{{ user.city }}{{ user.area }}{{ user.town }}{{ user.street
			}}{{ user.housenumber
			}}{{ user.address }}
		</div>
  ...
  new Vue({
    ...
    methods: {
      comfi_sub() {
        ...
        let repairdata = JSON.parse(localStorage.getItem('repairdata'));
        axios.post('/index.php/api/RecyclingMaterials', {
          userid: repairdata.userid,
          id: repairdata.id,
          serial: repairdata.serial,
          memberid: repairdata.memberid,
          customertype: repairdata.customertype,
          attributiondepartment: repairdata.attributiondepartment,
          packingtype: this.packingtype,
          num: this.num,
          brokerage: repairdata.maintenanceman,
        }).then(rew => {
