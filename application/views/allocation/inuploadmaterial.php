	...
	<div class="form-inline">
		...
		<div class="input-group mb-2 mr-sm-2">
			...
			<select name="" id="" v-model="jsr">
				<option  v-if="v.quartersid == 7" v-for="v in initData.Operator.info" :value="v">{{ v.name }}</option>



					<option v-for="v in AreaDeliverymanList" :value="v">{{ v.name }}</option>
...
<script>
	new Vue({
		...
		data: {
			...
			mode: '出库',
			goodsnum: '',
			AreaDeliverymanList: <?= json_encode($_SESSION['AreaDeliverymanList']) ?>
		},
		methods: {
			carchange(data) {
				console.log(data)
			...
			upload() {
				...
				axios.post('/index.php/api/sfuploadmaterinfo', {
