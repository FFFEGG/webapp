...
<div class="oapd">
	...
	<input type="hidden" name="info" value="<?= $this->input->get('data') ?>">
			...
			<select name="receiver" id="">
				<?php foreach ($_SESSION['initData']->Department->info as $v) { ?>
					<?php if ($v->type == '业务门店' || $v->type == '业务公司') { ?>
						<option <?php if ($info['source'] == $v->name) {
							echo 'selected';
	...
	<?php foreach ($info['goodslist'] as $k => $v) { ?>
		<div class="layui-form-item <?php if ($k == 0) {
			echo 'goods';
		} ?>">
			<div class="layui-inline">
