	...
	<script>
		new Vue({
			...
			watch: {
				...
				departmentname () {
					this.getgoodsprice(this.cardid,this.departmentname,this.YhGoods)
				}
			},
			computed: {
				lasttel() {
			...
			methods: {
						...
						for (var g = 0; g < this.GoodsList.length; g++) {
							...
							if (this.GoodsList[g]['id'] == data['goodsid']) {
								...
								if (this.GoodsList[g]['yhprice']) {
									...
									this.GoodsList[g]['price'] = this.GoodsList[g]['yhprice'] > useryhprice ? useryhprice : this.GoodsList[g]['yhprice']
				...
				getgoodsprice (memberid,did,yhgoods) {
					axios.post('/index.php/api/getusergoodsprice',{
						memberid: memberid,
						UserOrderCondition: yhgoods,
						departmentid: did
					}).then(rew=>{
						this.GoodsList = rew.data.list
					})
				}
