		...
		<table class="layui-table" lay-size="sm">
			...
			<tr>
				<?php foreach ($title as $v) { ?>
					<?php if ($v != '公司ID' && $v != '订单号' && $v != '用户ID'  && $v != '录入信息' ) { ?>
						<th><?= $v ?></th>
					<?php } ?>
				<?php } ?>
			</tr>
			...
			<?php foreach ($list as $v) {?>
				<tr>
					<?php foreach ($key as $vo) { ?>
						<?php if ($vo != 'companyid' && $vo != 'serial'&& $vo != 'userid'&& $vo != 'allcode') { ?>
						<?php if ($vo == 'state') { ?>
							<td><?= getstate($v->$vo) ?></td>
						<?php } elseif ($vo == 'goodsid') { ?>
							<td><?= getoneGoodsById($v->$vo)['name'] ?></td>
						<?php } elseif ($vo == 'memberid') { ?>
							<td><a style="cursor: pointer" onclick="Win10_child.openUrl('/index.php/order/lists?cardid=<?= $v->$vo ?>','预约下单','max')"><?= $v->$vo ?></a></td>
						<?php } else { ?>
							<td><?= $v->$vo ?></td>
						<?php } ?>
						<?php } ?>
