...
<div id="app" class="p-4">
	...
	<div class="px-12 mb-8">
		<div class="grid grid-cols-5 mb-4 text-2xl">
			...
			<p></p>
			<button onclick="window.location.reload()" class="bg-red-500 text-sm text-white rounded p-2 w-20">刷新</button>
		</div>

		<div class="grid grid-cols-10">
			<p v-for="v in order" class="mr-2">{{v.department}}:  {{v.aggregateorder}}</p>
		</div>
	</div>
...
<script>
  new Vue({
    ...
    methods: {
      getlist() {
        axios.post('/index.php/api/RealTimeDynamicOrder').then(rew => {
