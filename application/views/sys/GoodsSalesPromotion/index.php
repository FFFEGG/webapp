...
<div class="oapd">
	...
	<div class="layui-form-item layui-form">
		...
		<form action="" method="get">
			<div class="layui-inline">
				<button class="layui-btn" type="button" onclick="Win10_child.openUrl('/index.php/sys/GoodsSalesPromotion_form?action=ADD','商品促销方案')">新增</button>
			</div>
		</form>
			...
			<?php foreach ($info['data']['info'] as $v) {?>
				<tr>
					<?php foreach ($info['data']['key'] as $vo) {?>
						...
						<?php } elseif ($vo == 'departmentid') { ?>
							<td><?= getDepartmentById($v[$vo])['name'] ?></td>
						<?php }else{?>
							<td><?= $v[$vo]?></td>
						<?php }?>
					<?php }?>
					<td><button onclick="Win10_child.openUrl('/index.php/sys/GoodsSalesPromotion_form?action=UPDATE&id=<?= Myencode($v)?>','商品促销方案')" class="layui-btn">修改</button></td>
				</tr>
			<?php }?>
			</tbody>
		</table>
