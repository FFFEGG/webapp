	...
	<div class="layui-form-item layui-form">
		...
		<div class="layui-inline">
			<button class="layui-btn" type="button"
					onclick="Win10_child.openUrl('/index.php/sys/FeiePrint_form?action=ADD','飞鹅打印机接口')">新增
			</button>
		</div>
	</div>
			...
			<?php foreach ($info['data']['info'] as $v) {?>
				<tr>
					<?php foreach ($info['data']['key'] as $vo) {?>
						...
						<?php }?>
					<?php }?>
					<td><button onclick="Win10_child.openUrl('/index.php/sys/FeiePrint_form?action=UPDATE&id=<?= Myencode($v)?>','车辆信息')" class="layui-btn">修改</button></td>
				</tr>
			<?php }?>
			</tbody>
		</table>
