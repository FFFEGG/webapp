	...
	<div class="layui-form-item layui-form">
		...
		<div class="layui-inline">
			<button class="layui-btn" type="button"
					onclick="Win10_child.openUrl('/index.php/sys/SecurityCheckProject_form?action=ADD','安检项目')">新增
			</button>
		</div>
	</div>
			...
			<?php foreach ($info['data']['info'] as $v) { ?>
				<tr>
					<?php foreach ($info['data']['key'] as $vo) { ?>
						...
						<?php } elseif ($vo == 'typeid') { ?>

							<td><?= getSecurityCheckTypeId($v[$vo])['name'] ?></td>
					...
					<td>
						<button
							onclick="Win10_child.openUrl('/index.php/sys/SecurityCheckProject_form?action=UPDATE&id=<?= Myencode($v) ?>','安检项目')"
							class="layui-btn">修改
						</button>
					</td>
				</tr>
