	...
	<div class="layui-form-item layui-form">
		...
		<div class="layui-inline">
			<button class="layui-btn" type="button"
					onclick="Win10_child.openUrl('/index.php/sys/TwohundredItem_form?action=ADD','双百选项')">新增
			</button>
		</div>
	</div>
			...
			<?php foreach ($info['data']['info'] as $v) { ?>
				<tr>
					<?php foreach ($info['data']['key'] as $vo) { ?>
						<?php if ($vo == 'state') { ?>
							<td><?= getstate($v[$vo]) ?></td>
						<?php } elseif ($vo == 'twohundredtypeid') { ?>

							<td><?= getTwohundredtypeid($v[$vo])['name'] ?></td>
					...
					<td>
						<button
							onclick="Win10_child.openUrl('/index.php/sys/TwohundredItem_form?action=UPDATE&id=<?= Myencode($v) ?>','双百选项')"
							class="layui-btn">修改
						</button>
					</td>
				</tr>
