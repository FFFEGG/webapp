...
<div id="app" class="p-6">
	<div class="flex">
		...
		<select class="p-2 border-2 rounded-r" name="" id="" v-model="department">
			<option value="全部">全部</option>
			<option v-if="v.type == '业务门店'" v-for="v in initData.Department.info" :value="v.name">{{v.name}}</option>
		</select>
	...
	<table class="table-auto mt-6 w-full text-xs">
		...
		<tr class="text-center" v-for="v in list">
			...
			<td class="border-2 ">
				<button @click="TPrint(v)" class="p-2 bg-teal-500 text-white">打印</button>
			</td>
		</tr>
...
<script>
	new Vue({
		...
		methods: {
			submit() {
				axios.post('/index.php/api/TReprintCodeRecord', {
