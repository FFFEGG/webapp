define('script/modules/trackcontrol/stores/trackStore', function(require, exports, module) {
  ...
  var TrackStore = Reflux.createStore({
      ...
      data: {
          ...
          transport_mode: ['driving', 'riding', 'walking'],
          // 异步加载的驾驶分析排序数据
       ...
       */
      onSwitchboundsearch: function onSwitchboundsearch(data) {
       ...
       */
      onBoundsearchentity: function onBoundsearchentity(display) {
          var that = this;
          var time = new Date();
          that.data.boundsearchTimestamp = time.getTime();
          var inTimestamp = that.data.boundsearchTimestamp;
          var inBoundsEntity = [];
          var inBoundsEntityCount = 0;
          var total = 0;
          var boundsLimit = 50000;
          ...
          var lngLatToPoint = function lngLatToPoint(lnglat) {
              return map.getMapType().getProjection().lngLatToPoint(lnglat);
          };
          var pointToLngLat = function pointToLngLat(point) {
          ...
          var getBoundsearchResultTable = function getBoundsearchResultTable() {
              boundsArr.map(function (item, index) {
                  ...
                  _commonUrls2['default'].jsonp(_commonUrls2['default'].boundsearchEntity, params, function (data) {
                      if (data.status === 0) {
                          data.entities.map(function (eitem, eindex) {
                              var status = '';
                              var entity_status = 0;
                              if (_commonCommonfun2['default'].getOnlineStatus(eitem.latest_location.loc_time) === 0) {
                                  eitem.latest_location.speed = eitem.latest_location.speed || 0;
                                  status = _commonCommonfun2['default'].getSpeed(eitem.latest_location.speed);
          ...
          var getBoundsearchResultOnce = function getBoundsearchResultOnce(page_index) {
          ...
          if (boundsDistance > width * boundsLimit) {
              var _ret = (function () {
                  var tempData = [];
                  var tempCount = 0;
                  var tempTotal = that.data.entityTotal[that.data.boundsType];
       ...
       *
       */
      onSelectcar: function onSelectcar(entity_name, entity_status, entity_id) {
          var that = this;
                      ...
                      if (item.entity_name === entity_name) {
                          ...
                          _commonUrls2['default'].jsonp(_commonUrls2['default'].getAddress, paramsGeo, function (dataGeo) {
                              ...
                              that.data.selectCompleteEntities[index] = {
                                  ...
                                  status: _commonCommonfun2['default'].getInfoWindowStatus(item.latest_location.speed, item.latest_location.loc_time, item.latest_location.direction),
                                  infor: [['状态:', _commonCommonfun2['default'].getInfoWindowStatus(item.latest_location.speed, item.latest_location.loc_time, item.latest_location.direction)], ['地址:', address], ['定位:', lnglat], ['时间:', _commonCommonfun2['default'].getLocalTime(item.latest_location.loc_time)]].concat(temp)
                              };
                              that.data.selectCarStartDatetime = _commonCommonfun2['default'].getStartTime(new Date(item.latest_location.loc_time * 1000));
                              // 从localstorage中获取应该显示到列表中的字段，entitydesc或者entityname
                              var entityOption = localStorage['entityOption' + that.data.serviceId];
       ...
       *
       */
      onClosemonitorinfobox: function onClosemonitorinfobox() {
      ...
      setAllEntities: function setAllEntities(data) {
          ...
          data.entities.map(function (item, index) {
              ...
              item.entity_name, desc, descIndex, '', item.entity_desc ? item.entity_desc : '无'];
              ...
              if (data.status === 0) {
                  ...
                  data.entities.map(function (item) {
                      var paramsd = {
                          ...
                          'process_option': 'need_denoise=' + that.data.trackProcess.need_denoise + ',' + 'need_vacuate=' + that.data.trackProcess.need_vacuate + ',' + 'need_mapmatch=' + that.data.trackProcess.need_mapmatch + ',' + 'transport_mode=' + that.data.transport_mode[that.data.trackProcess.transport_mode - 1]
                      };
                      _commonUrls2['default'].jsonp(_commonUrls2['default'].getDistance, paramsd, function (datad) {
                          if (datad.status === 0) {
                              ...
                              tempTimeArr.map(function (item_time) {
                                  ...
                                  _commonUrls2['default'].jsonp(_commonUrls2['default'].getDistance, param_time, function (data_time) {
      ...
      * @param {string} data entity数据
      ...
      onSelecttrack: function onSelecttrack(data) {
          ...
          var reTrackRoute = function reTrackRoute(paramsr, page_index) {
              ...
              var search = function search(paramsearch, counta, countb) {
                  _commonUrls2['default'].jsonp(_commonUrls2['default'].getTrack, paramsearch, function (data) {
       ...
       */
      onChangeTimeline: function onChangeTimeline(starttime, endtime) {
       ...
       */
      onGetaddress: function onGetaddress(point) {
          var that = this;
          var param = {
              location: point.lat + ',' + point.lng,
              output: 'json'
          };
          _commonUrls2['default'].jsonp(_commonUrls2['default'].getAddress, param, function (data) {
              var infoBoxObject = that.getTrackPointInfo(data, point);
       ...
       */
      onTriggerswitchmanagetab: function onTriggerswitchmanagetab(index) {
       ...
       *
       */
      onTriggersearchentitytrack: function onTriggersearchentitytrack() {
       ...
       *
       */
      onTriggersetdate: function onTriggersetdate() {
       ...
       *
       */
      onTriggerselecttrack: function onTriggerselecttrack() {
       ...
       */
      getTrackPointInfo: function getTrackPointInfo(data, point) {
          var address = '';
          if (data.status === 0) {
          ...
          var infoBoxObject = {
              point: point,
              print: point.print,
              infor: [['定位:', point.lnglat], ['地址:', address], ['速度:', point.printSpeed], ['时间:', point.printTime], ['高度:', point.height + '米'], ['精度:', point.radius + '米']]
              // lnglat: point.lnglat,
              // address: address,
  ...
  //# sourceMappingURL=/script/modules/trackcontrol/stores/trackStore.js.map
